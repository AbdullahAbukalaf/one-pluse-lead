...
<div class="container-fluid">
    <h4 class="mb-3">Insights Section</h4>


    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route('admin.insights.section.update') }}" enctype="multipart/form-data">
                @csrf

                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="form-label">Outline Title EN</label>
                        <input type="text" name="outline_title_en" class="form-control" value="{{ old('outline_title_en', $item->outline_title_en ?? '') }}">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Outline Title AR</label>
                        <input type="text" name="outline_title_ar" class="form-control" value="{{ old('outline_title_ar', $item->outline_title_ar ?? '') }}">
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        @if(!empty($item?->image))
                            <div class="mt-2">
                                <img src="{{ asset('storage/' . $item->image) }}" alt="Image" style="max-width:240px;border-radius:8px;">
                            </div>
                        @endif
                    </div>
